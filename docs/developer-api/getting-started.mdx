...
Limits are counted per API key, in three separate buckets. Every endpoint in a bucket shares that bucket's count.

| Bucket | Endpoints | Limit |
|---|---|---|
| Reads | Every `GET` (workspace, chatbots, flows, conversations, integrations, knowledge bases, analytics) | 60 requests per 60 seconds |
| Writes | Every `POST`, `PUT`, `PATCH` and `DELETE` on those resources, including publishing a flow and sending an outbound message | 30 requests per 60 seconds |
| Chat | Creating a chat session and sending a message to it ([Chat API](./chat)) | 600 requests per 60 seconds |

If you exceed a limit you receive `429 Too Many Requests` with `detail` `Too many requests. Please try again later.` and a `Retry-After` header giving the seconds to wait.
