...
Session creation only initializes the session — no messages are returned. Send the user's first message to [Send Message](#send-message) (`POST /api/dev/v1/chatbots/{chatbot_id}/chat/sessions/{session_id}/messages`) to start the flow. The response to that first message contains the flow's opening messages (the same messages a user sees when opening the chat widget), following the GenericMessage structure.
